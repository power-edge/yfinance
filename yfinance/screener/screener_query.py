...
class Query(ABC):
    ...
    def to_dict(self) -> Dict:
        raise YFNotImplementedError(
            "to_dict() needs to be implemented by children classes"
        )
...
class EquityQuery(Query):
    ...
    def __init__(
        self, operator: str, operand: Union[numbers.Real, str, List["EquityQuery"]]
    ):
        """
        .. seealso::
    ...
    @dynamic_docstring(
        {
            "valid_operand_fields_table": generate_list_table_from_dict(
    ...
    def _validate_eq_operand(self, operand: List[Union[str, numbers.Real]]) -> None:
        ...
        if not any(
            ...
            for fields_by_type in EQUITY_SCREENER_FIELDS.values()
        ):
            raise ValueError("Invalid field for Screener")
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "operands": [
                operand.to_dict() if isinstance(operand, EquityQuery) else operand
