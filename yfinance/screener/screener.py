...
class Screener:
    ...
    @dynamic_docstring(
        {
            "predefined_screeners": generate_list_table_from_dict_of_dict(
                PREDEFINED_SCREENER_BODY_MAP, bullets=False
            )
        }
    )
    @property
    ...
    def set_predefined_body(self, predefined_key: str) -> "Screener":
        ...
        if not body:
            raise ValueError(
                f"Invalid key {predefined_key} provided for predefined screener"
            )
    ...
    def _validate_body(self) -> None:
        ...
        if self._body["size"] > 250:
            raise ValueError(
                "Yahoo limits query size to 250. Please decrease the size of the query."
            )
    ...
    def _fetch_and_parse(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to get screener data for '{self._body.get('query', 'query not set')}' reason: {e}"
            )
