...
class PriceHistory:
    ...
    def history(
        ...
        if data is None or not isinstance(data, dict):
            ...
            _exception = YFInvalidPeriodError(
                self.ticker, period, self._history_metadata["validRanges"]
            )
            fail = True
        ...
        df = utils.fix_Yahoo_returning_live_separate(
            df, params["interval"], tz_exchange, repair=repair, currency=currency
        )
        ...
        if f.any():
            ...
            for i in indices:
                ...
                if i > 0:
                    ...
                    if (
                        abs(div_dt - prev_div.name) <= phantom_proximity_threshold
                        and not prev_div["phantom"]
                        and divergence < 0.01
                    ):
                    ...
                    divergence = min(abs(ratio1 - 1.0), abs(ratio2 - 1.0))
                    if (
                        abs(div_dt - next_div.name) <= phantom_proximity_threshold
                        and divergence < 0.01
                    ):
        ...
        for i in range(len(div_status_df)):
            ...
            if lookahead_idx > lookback_idx:
                ...
                for i in np.where(x["Dividends"] > 0)[0]:
                    x.loc[x.index[i], "adjDelta"] += (
                        x["Dividends"].iloc[i] * x["Adj"].iloc[i]
                    )
    ...
    def _fix_prices_sudden_change(
        ...
        if f_down.any():
            ...
            if f_down.ndim == 2:
                f_div_before = f_div_before[:, np.newaxis].repeat(
                    f_down.shape[1], axis=1
                )
