            ...
            for i in range(len(intervals)):
                ...
                for j in range(i, len(periods)):
                    ...
                    if len(dfr) != len(df_truth):
                        ...
                        vol_diff_pct0 = (
                            dfr["Volume"].iloc[0] - df_truth["Volume"].iloc[0]
                        ) / df_truth["Volume"].iloc[0]
                        vol_diff_pct1 = (
                            dfr["Volume"].iloc[-1] - df_truth["Volume"].iloc[-1]
                        ) / df_truth["Volume"].iloc[-1]
...
class TestPriceRepair(unittest.TestCase):
    ...
    def test_repair_zeroes_daily_adjClose(self):
        ...
        df = _pd.DataFrame(
            data={
                "Open": [28.95, 28.65, 29.55, 29.62, 29.25],
    ...
    def test_repair_bad_div_adjusts(self):
        ...
        bad_tkrs += [
            "SOLB.BR"
        ]  # 1x phantom div, but some false-positive 100x. and had to improve phantom detection
        bad_tkrs += ["TEM.L"]
        bad_tkrs += ["TEP.PA"]
